...
export default function ListaProdutoPremium({ p, onComparar, comparado }: {
  ...
  const oferta = p.precoOferta && Number(p.precoOferta) > 0;
  const desconto = oferta ? Math.round(((Number(p.precoVenda) - Number(p.precoOferta)) / Number(p.precoVenda)) * 100) : 0;
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 hover:border-brand-300 hover:shadow-md transition-all duration-200 p-4 flex gap-4 group">
      ...
      <a href={`/vitrine/produto/${p.id}`} className="w-28 h-28 rounded-lg bg-slate-50 flex-shrink-0 overflow-hidden">
        {p.imagemUrl ? (
          <img src={p.imagemUrl} alt={p.nome} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" loading="lazy" />
        ) : (
        ...
        <div className="flex items-center justify-between mt-2">
          <div>
            <div className="flex items-baseline gap-2">
              ...
              {oferta && <span className="text-xs text-slate-400 line-through">{fm(Number(p.precoVenda))}</span>}
            </div>
