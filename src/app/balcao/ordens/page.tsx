...
export default function OrdensPage() {
  ...
  async function criarOS() {
    if (!form.nomeCliente || !form.telefoneCliente || !form.modeloMoto || !form.descricaoProblema) { setMsg('Preencha os campos obrigatorios.'); return; }
...
function DetalheOSBalcao({ os, onClose }: { os: OS; onClose: () => void }) {
  ...
  async function atualizarStatus() { const res=await fetch(`/api/ordens/${dados.id}/status`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({status:novoStatus,mecanicoId:mecId||null,diagnostico:diagnostico||null})});if(res.ok){const u=await res.json();setDados(u);setMsg('');}else{const e=await res.json();setMsg(e.error||'Erro.');} }
  ...
  return (
    <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4"><div className="bg-white rounded-xl border border-slate-200 shadow-xl w-full max-w-2xl max-h-[85vh] flex flex-col">
      <div className="p-5 border-b border-slate-100 flex items-center justify-between flex-shrink-0"><div><h2 className="text-base font-semibold text-slate-800">OS #{dados.numero}</h2><p className="text-sm text-slate-500">{dados.nomeCliente} - {dados.modeloMoto}{dados.placaMoto?` (${dados.placaMoto})`:''}</p></div><button onClick={onClose} className="text-slate-400 hover:text-slate-600 text-lg">&times;</button></div>
      <div className="flex border-b border-slate-100 flex-shrink-0">{['itens','status','nf'].map(t=>(<button key={t} onClick={()=>setTab(t as any)} className={`px-4 py-2 text-xs font-medium border-b-2 transition-colors ${tab===t?'border-brand-600 text-brand-600':'border-transparent text-slate-500 hover:text-slate-700'}`}>{t==='itens'?'Pecas':t==='status'?'Status':'Nota Fiscal'}</button>))}</div>
      <div className="p-5 overflow-y-auto flex-1">{msg&&<div className="bg-red-50 border border-red-200 text-red-700 px-3 py-2 rounded text-xs mb-4">{msg}</div>}
        {tab==='itens'&&(<div>
