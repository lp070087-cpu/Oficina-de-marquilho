...
export default function EstoqueDashboardPage() {
  ...
  const tipoLabel: Record<string,string> = { ENTRADA:'text-emerald-700 bg-emerald-50', SAIDA:'text-red-700 bg-red-50', TRANSFERENCIA:'text-brand-700 bg-brand-50', VENDA:'text-amber-700 bg-amber-50', USO_OS:'text-orange-700 bg-orange-50' };
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[{ label:'Produtos cadastrados', value:stats.totalProdutos, sub:null },{ label:'Quantidade total', value:stats.totalUnidades, sub:'unidades' },{ label:'Estoque minimo', value:stats.estoqueMinimo, sub:'precisa repor' },{ label:'Sem estoque', value:stats.semEstoque, sub:'zerados' },{ label:'Entradas hoje', value:stats.entradasHoje, sub:null },{ label:'Transferencias hoje', value:stats.transferenciasHoje, sub:null }].map((c,i)=>(
      ...
      <div className="card-table">
        ...
        <div className="overflow-auto max-h-80"><table className="w-full text-xs">
          <thead><tr className="border-b border-slate-100 bg-slate-50/60">
            <th className="text-left py-2 px-3 font-semibold text-slate-500">Data</th><th className="text-left py-2 px-3 font-semibold text-slate-500">Tipo</th><th className="text-left py-2 px-3 font-semibold text-slate-500">Produto</th><th className="text-center py-2 px-3 font-semibold text-slate-500">Qtd</th><th className="text-left py-2 px-3 font-semibold text-slate-500">Usuario</th></tr></thead>
          <tbody>{stats.ultimasMovimentacoes.length===0?<tr><td colSpan={5} className="py-8 text-center text-slate-400">Nenhuma movimentacao registrada.</td></tr>:
            stats.ultimasMovimentacoes.map((m:any,i:number)=>(<tr key={i} className={`border-b border-slate-50 ${i%2===0?'bg-white':'bg-slate-50/20'}`}><td className="py-2 px-3 text-slate-500">{new Date(m.createdAt).toLocaleDateString('pt-BR')}</td><td className="py-2 px-3"><span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold ${tipoLabel[m.tipo]||'bg-slate-50 text-slate-500'}`}>{m.tipo}</span></td><td className="py-2 px-3 text-slate-700 font-medium">{m.peca?.nome||'-'}</td><td className="py-2 px-3 text-center font-bold">{m.quantidade}</td><td className="py-2 px-3 text-slate-400">{m.usuario||'-'}</td></tr>))}</tbody>
        </table></div>
      </div>
    </div>
  );
