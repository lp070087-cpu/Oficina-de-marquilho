...
export default function OrdensPage() {
  ...
  const fetchOrdens = useCallback(async () => {
    const statusMap: Record<string, string> = { ABERTAS: '', ABERTA: 'ABERTA', EM_ANDAMENTO: 'EM_ANDAMENTO', AGUARDANDO_PECAS: 'AGUARDANDO_PECAS', CONCLUIDAS: 'CONCLUIDA', ENTREGUES: 'ENTREGUE' };
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        {['ABERTAS','ABERTA','EM_ANDAMENTO','AGUARDANDO_PECAS','CONCLUIDAS','ENTREGUES'].map(f => (<button key={f} onClick={()=>setFiltro(f)} className={`px-3 py-1.5 rounded text-xs font-medium border transition-colors ${filtro===f?'bg-brand-600 text-white border-brand-600':'bg-white text-slate-600 border-slate-200 hover:border-slate-300'}`}>{statusLabel[f]||'Todas'}</button>))}
      </div>
