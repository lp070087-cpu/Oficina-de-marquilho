...
export default function EstoqueLoginsPage() {
  ...
  async function fetchUsers() {
    ...
    setUsers(data.filter((u: User) => u.role === 'ESTOQUE'));
    setLoading(false);
  }
  ...
  async function salvar() {
    ...
    if (editando) {
      ...
      const res = await fetch('/api/usuarios', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({...form, role:'ESTOQUE'}) });
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-slate-800 tracking-tight">ESTOQUE - LOGINS</h1>
          <p className="text-sm text-slate-500 mt-0.5">Computador do estoque — ve o que saiu e relatorios</p>
        </div>
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xl w-full max-w-sm p-6">
            ...
            <div className="space-y-4">
              <div><label className="text-xs font-semibold text-slate-600 uppercase tracking-wider">Nome</label><input value={form.name} onChange={e=>setForm({...form,name:e.target.value})} className="input-field mt-1.5" placeholder="Ex: Joao - Estoque"/></div>
              <div><label className="text-xs font-semibold text-slate-600 uppercase tracking-wider">Email de login</label><input value={form.email} onChange={e=>setForm({...form,email:e.target.value})} className="input-field mt-1.5" placeholder="joao.estoque@loja.com" disabled={!!editando}/></div>
              <div>
