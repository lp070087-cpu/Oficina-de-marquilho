...
export default function NotasPage() {
  ...
  async function fetchData() {
    ...
    if (res) {
      ...
      setOrdens(data.filter((o:OS) => o.status==='CONCLUIDA'));
    }
    setLoading(false);
  }
  useEffect(() => { fetchData(); }, []);
          ...
          <table className="w-full text-sm">
            ...
            <tbody>{ordens.map(os => (
              <tr key={os.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                ...
                <td className="py-2 px-3 text-xs text-slate-500">{new Date(os.createdAt).toLocaleDateString('pt-BR')}</td>
                <td className="py-2 px-3 text-center">
                  <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold ${os.statusPagamento==='PAGO'?'bg-emerald-50 text-emerald-700':'bg-red-50 text-red-700'}`}>
                    {os.statusPagamento==='PAGO'?'🟢 Pago':'🔴 Pendente'}
                  </span>
                </td>
                <td className="py-2 px-3 text-right">
                  {os.statusPagamento!=='PAGO' && (
                    <button onClick={()=>marcarPago(os.id)} className="text-xs bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-1 rounded font-bold">
                      Marcar Pago
                    </button>
                  )}
                </td>
